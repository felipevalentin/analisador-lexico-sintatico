...
class State:
    def __init__(self, alphabet, id_list, id, terminal_id):
        ...
        self.transitions = (
            dict()
        )  # Dictionary to keep all the transitions to other states
        self.final = terminal_id in self.id_set  # True if this is a final state
        for a in alphabet:
            self.transitions[
                a
            ] = {}  # Each transitions from this state by a char is stored in a set
...
class DFA:
    def __init__(self, alphabet, tree):
        ...
        self.terminal = (
            tree.id_counter - 1
        )  # '#' leaf is the end of regex and the id of it is assigned to terminal
    ...
    def Dtran(self, state, tree):
        ...
        for i in state.id_set:
            ...
            if state.transitions[label] == {}:
                ...
                state.transitions[label] = state.transitions[label].union(
                    tree.followpos[i]
                )
        for a in self.alphabet:
            if state.transitions[a] != {}:
                ...
                for s in self.states:
                    if (
                        s.id_set == state.transitions[a]
                        or state.transitions[a] in new_states
                    ):
                        new = False
                if new:

...
class Tabela_Lexica:
    def __init__(self, p_r, automatos, automato_geral, regex):
        ...
        self.estado_tokens = (
            []
        )  # lista dos estados finais do automato final e o padrão que ele gera
    ...
    def ler_palavra(self, input_):
        ...
        for p in range(0, len(input_)):
            for i in range(0, len(self.automato_geral.transicoes)):
                ...
                if (
                    state == self.automato_geral.transicoes[i][0]
                    and input_[p] == self.automato_geral.transicoes[i][1]
                ):
            ...
            for e in range(0, len(self.automato_geral.estados_finais[i])):
                ...
                for a in range(0, len(self.automatos)):
                    ...
                    for f in range(0, len(self.automatos[a].estados_finais)):
                        if (
                            self.automato_geral.estados_finais[i][e]
                            == self.automatos[a].estados_finais[f]
                        ):
                            padrao = self.automatos[a].def_re
                            estado_token = (
                                self.automato_geral.estados_finais[i],
                                padrao,
                            )
                if padrao != "":
                ...
                for a in range(1, (len(self.automato_geral.alfabeto) + 1)):
                    ...
                    for t in range(0, len(self.automato_geral.transicoes)):
                        if states[s - 1] == self.automato_geral.transicoes[t][0]:
                            if (
                                self.automato_geral.transicoes[t][1]
                                == self.automato_geral.alfabeto[a - 1]
                            ):
    ...
    def salvar_tabela(self):
        ...
        with open("tabela_analise.txt", "w") as f:
            ...
            for state in states:
                for estado_token in self.estado_tokens[::-1]:
                    if state in estado_token:
                        f.write(
                            f"{str(estado_token[1]) + ' ' +  str(state):<{largura_maior_estado}}"
                        )
                        break
                else:
...
def main():
    ...
    if tabela.gera_lista() == -1:
        raise Exception("Invalido: Erro léxico")
    tabela.output_token()
